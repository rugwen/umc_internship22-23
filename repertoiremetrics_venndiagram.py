...
file_name = r"/data\NGSdata2.xlsx"

venn_file_name = r"/data\venn_data.xlsx"
...
fig = ax.get_figure()
fig.savefig(location+"Clonality_s.png",bbox_inches = "tight")
...
plt.legend(handles=[blue_patch, red_patch], loc = "best")
fig = ax2.get_figure()
fig.savefig(location+"Shannon_s.png",bbox_inches = "tight")
...
if sig:
    ...
    fig.savefig(location+"Chao1_s.png", bbox_inches = "tight")
...
for patient in overlapping:
    ...
    try:
        ...
        venn3([set(venndf[x[0]]),set(venndf[x[1]]),set(venndf[x[2]])], set_labels=("c1", "c2", "FFPE"), set_colors=colors4,
             subset_label_formatter=lambda x: str(x) + "\n" + f"{x/total:1.0%}" + "",
             ax=axes[1])
    if patient == "080" or patient == "022":
        fig.suptitle(f"Responder {patient}", fontsize = 14,y=0.9)
    else:
        fig.suptitle(f"Non-responder {patient}", fontsize = 14,y=0.9)
    plt.savefig(rf"/data\venn\{patient}_clonotype.png", dpi=300)
    plt.savefig(rf"/data\venn\{patient}_clonotype.svg", dpi=300)
